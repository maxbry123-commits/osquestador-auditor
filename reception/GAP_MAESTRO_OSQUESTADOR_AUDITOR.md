...
### 3.8b App/Web Generator Engine — ★ FALTABA EN ESTA LISTA, corregido
### 2026-07-21: es la familia MÁS PROBADA del sistema (Salida 4, servidor
### FastAPI real corrió, curl real, tests PASS) y no tenía casilla propia
Scaffold Engine✅(=ensamblador.py) · Backend Generator✅(FastAPI+SQLite) ·
Frontend Generator✅(HTML/JS vanilla) · Component Library
Generator🔲(G1) · Auth Generator🔲(G2) · DB Production Generator🔲(G3) ·
Integration Generator🔲(G4, email/pagos/storage/push) · Test Generator
Ampliado🔲(G5) · CI/CD Generator🔲(G6) · Observability Generator🔲(G7) ·
Quality Auditor🔲(G8, seguridad/SEO/accesibilidad) · Mobile Generator
🔲(Fase B, Flutter) · PWA Generator🔲(Fase A)
*(detalle completo de G1-G8 en ANALISIS_BRECHAS_WEB_APP_100.md — no se
...
## 8. ORDEN UNIFICADO DE PRIORIDAD (arquitectura + generador, UNA sola
## lista — corrige que antes había 2 recomendaciones sueltas compitiendo)

```
1. Council (Consensus Runtime, sección 2.S)
   → SIN esto, cada pieza nueva (incluyendo G1-G8) se integra sin
   validación obligatoria — es la base que falta antes de seguir
   agregando generadores

2. G1 — Selector de Stack + Biblioteca de Componentes
   (ANALISIS_BRECHAS_WEB_APP_100.md) → primer generador nuevo, y el
   Council recién construido lo valida desde el día 1

3. Meta Kernel — separar formalmente de memoryos.py (sección 2.A)
   → antes de seguir metiendo motores nuevos (G2 en adelante), conviene
   que el registro/descubrimiento quede ordenado

4. G2 → G8 en el orden ya definido en ANALISIS_BRECHAS_WEB_APP_100.md
   (sección 5 de ese documento), cada uno pasando por el Council

5. Universal Contract (sección 2.B) — retrofit a los 20+ motores ya
   construidos, en paralelo mientras avanza el punto 4
```

**Regla de consistencia:** cualquier pieza nueva de aquí en adelante se
valida contra ESTA sección 8, no contra recomendaciones sueltas del
chat — así no vuelven a competir 2 prioridades sin resolver.

---

## 9. GAPS FUERA DEL OSQUESTADOR (resto de NCT — no estaban en el
## documento, se agregan ahora tras auditoría, con corrección de un
## error propio)

| Área | Estado REAL verificado en este chat | Corrección |
|---|---|---|
| Router R1-R4 | ✅ **CONSTRUIDO** (Salida 2: BYOK+capability+batching+ salud+Cost Optimizer+7 conectores, 20 tests) | corrige afirmación errónea hecha en el chat de que estaba "parcial" — no volver a repetir ese error |
| Router R5-R8 | ✅ construido (Grupo 2, retry+circuit breaker+caché) | — |
| 325 fichas del sistema (E/P/S/T) | 🔲 casi sin tocar — solo renumeración pendiente mencionada en memoria antigua, nunca ejecutada en este chat | sigue siendo un gap real |
| Despliegue real a GitHub/VPS | ❌ **nunca ha ocurrido en esta conversación** — todo lo probado es local (`/tmp/...`) o `--dry-run`. Ningún `git push` a un remoto real, ninguna credencial real usada | gap real, no exagerado |
| Paneles (Red/Auditor/Documentos/Maestro) | 🔷 4 HTML construidos, todos con `MODO_DEMO=true`; el `servidor_paneles.py` (G7C) se probó por `curl` directo, **nunca se probó el panel real contra el servidor real con MODO_DEMO=false** | gap real: falta la prueba end-to-end panel↔servidor |

**Nota de proceso:** este tipo de error (memoria vieja sin verificar
contra el chat actual) es justo lo que la Sección 8 (Council) está
diseñada para prevenir — un validador obligatorio habría exigido
evidencia antes de aceptar la afirmación "Router parcial".
