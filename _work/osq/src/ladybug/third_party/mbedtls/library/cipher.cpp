...
int mbedtls_cipher_set_iv(mbedtls_cipher_context_t* ctx, const unsigned char* iv, size_t iv_len) {
    ...
    if (MBEDTLS_MODE_GCM == ctx->cipher_info->mode) {
        return (
            mbedtls_gcm_starts((mbedtls_gcm_context*)ctx->cipher_ctx, ctx->operation, iv, iv_len));
    }
#endif
    ...
    if (MBEDTLS_MODE_CCM_STAR_NO_TAG == ctx->cipher_info->mode) {
        ...
        return (
            mbedtls_ccm_starts((mbedtls_ccm_context*)ctx->cipher_ctx, ccm_star_mode, iv, iv_len));
    }
#endif
    ...
    *olen = 0;
    ...
    if (ctx->cipher_info->mode == MBEDTLS_MODE_CBC) {
        ...
        if (0 != ctx->unprocessed_len) {
            ...
            if (0 != (ret = ctx->cipher_info->base->cbc_func(ctx->cipher_ctx, ctx->operation,
                          block_size, ctx->iv, ctx->unprocessed_data, output))) {
                return (ret);
            }
    ...
    if (ctx->cipher_info->mode == MBEDTLS_MODE_CFB) {
        if (0 != (ret = ctx->cipher_info->base->cfb_func(ctx->cipher_ctx, ctx->operation, ilen,
    ...
    if (ctx->cipher_info->mode == MBEDTLS_MODE_CTR) {
        if (0 != (ret = ctx->cipher_info->base->ctr_func(ctx->cipher_ctx, ilen,
                      &ctx->unprocessed_len, ctx->iv, ctx->unprocessed_data, input, output))) {
            return (ret);
        }
    ...
    if (ctx->cipher_info->mode == MBEDTLS_MODE_STREAM) {
        if (0 !=
            (ret = ctx->cipher_info->base->stream_func(ctx->cipher_ctx, ilen, input, output))) {
            return (ret);
        }
...
int mbedtls_cipher_finish(mbedtls_cipher_context_t* ctx, unsigned char* output, size_t* olen) {
    ...
    *olen = 0;
    ...
    if (MBEDTLS_MODE_CBC == ctx->cipher_info->mode) {
        ...
        /* cipher block */
        if (0 !=
            (ret = ctx->cipher_info->base->cbc_func(ctx->cipher_ctx, ctx->operation,
                 mbedtls_cipher_get_block_size(ctx), ctx->iv, ctx->unprocessed_data, output))) {
            return (ret);
        }
...
static int mbedtls_cipher_aead_encrypt(mbedtls_cipher_context_t* ctx, const unsigned char* iv,
    ...
    if (MBEDTLS_MODE_GCM == ctx->cipher_info->mode) {
        *olen = ilen;
        return (mbedtls_gcm_crypt_and_tag((mbedtls_gcm_context*)ctx->cipher_ctx,
            MBEDTLS_GCM_ENCRYPT, ilen, iv, iv_len, ad, ad_len, input, output, tag_len, tag));
    }
#endif /* MBEDTLS_GCM_C */
#if defined(MBEDTLS_CCM_C)
...
int mbedtls_cipher_auth_encrypt_ext(mbedtls_cipher_context_t* ctx, const unsigned char* iv,
    ...
    if (
        ...
        ctx->psa_enabled == 0 &&
#endif
        (MBEDTLS_MODE_KW == ctx->cipher_info->mode || MBEDTLS_MODE_KWP == ctx->cipher_info->mode)) {
        mbedtls_nist_kw_mode_t mode =
            (MBEDTLS_MODE_KW == ctx->cipher_info->mode) ? MBEDTLS_KW_MODE_KW : MBEDTLS_KW_MODE_KWP;
...
int mbedtls_cipher_auth_decrypt_ext(mbedtls_cipher_context_t* ctx, const unsigned char* iv,
    ...
    if (
        ...
        return (
            mbedtls_nist_kw_unwrap(ctx->cipher_ctx, mode, input, ilen, output, olen, output_len));
    }
#endif /* MBEDTLS_NIST_KW_C */
