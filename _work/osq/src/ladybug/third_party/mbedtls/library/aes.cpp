...
static const unsigned char AESFSb[256] = {0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30,
    ...
    0xA5, 0xE5, 0xF1, 0x71, 0xD8, 0x31, 0x15, 0x04, 0xC7, 0x23, 0xC3, 0x18, 0x96, 0x05, 0x9A, 0x07,
    0x12, 0x80, 0xE2, 0xEB, 0x27, 0xB2, 0x75, 0x09, 0x83, 0x2C, 0x1A, 0x1B, 0x6E, 0x5A, 0xA0, 0x52,
    0x3B, 0xD6, 0xB3, 0x29, 0xE3, 0x2F, 0x84, 0x53, 0xD1, 0x00, 0xED, 0x20, 0xFC, 0xB1, 0x5B, 0x6A,
    0xCB, 0xBE, 0x39, 0x4A, 0x4C, 0x58, 0xCF, 0xD0, 0xEF, 0xAA, 0xFB, 0x43, 0x4D, 0x33, 0x85, 0x45,
    ...
    0xD3, 0xAC, 0x62, 0x91, 0x95, 0xE4, 0x79, 0xE7, 0xC8, 0x37, 0x6D, 0x8D, 0xD5, 0x4E, 0xA9, 0x6C,
    0x56, 0xF4, 0xEA, 0x65, 0x7A, 0xAE, 0x08, 0xBA, 0x78, 0x25, 0x2E, 0x1C, 0xA6, 0xB4, 0xC6, 0xE8,
    0xDD, 0x74, 0x1F, 0x4B, 0xBD, 0x8B, 0x8A, 0x70, 0x3E, 0xB5, 0x66, 0x48, 0x03, 0xF6, 0x0E, 0x61,
    0x35, 0x57, 0xB9, 0x86, 0xC1, 0x1D, 0x9E, 0xE1, 0xF8, 0x98, 0x11, 0x69, 0xD9, 0x8E, 0x94, 0x9B,
    0x1E, 0x87, 0xE9, 0xCE, 0x55, 0x28, 0xDF, 0x8C, 0xA1, 0x89, 0x0D, 0xBF, 0xE6, 0x42, 0x68, 0x41,
...
int mbedtls_aes_setkey_enc(mbedtls_aes_context* ctx, const unsigned char* key,
    ...
    case 10:
        ...
        for (i = 0; i < 10; i++, RK += 4) {
            RK[4] = RK[0] ^ RCON[i] ^ ((uint32_t)AESFSb[MBEDTLS_BYTE_1(RK[3])]) ^
    ...
    case 12:
        ...
        for (i = 0; i < 8; i++, RK += 6) {
            RK[6] = RK[0] ^ RCON[i] ^ ((uint32_t)AESFSb[MBEDTLS_BYTE_1(RK[5])]) ^
    ...
    case 14:
        ...
        for (i = 0; i < 7; i++, RK += 8) {
            RK[8] = RK[0] ^ RCON[i] ^ ((uint32_t)AESFSb[MBEDTLS_BYTE_1(RK[7])]) ^
            ...
            RK[12] = RK[4] ^ ((uint32_t)AESFSb[MBEDTLS_BYTE_0(RK[11])]) ^
...
int mbedtls_aes_setkey_dec(mbedtls_aes_context* ctx, const unsigned char* key,
    ...
    *RK++ = *SK++;
    ...
    for (i = ctx->nr - 1, SK -= 8; i > 0; i--, SK -= 8) {
        for (j = 0; j < 4; j++, SK++) {
            *RK++ = AES_RT0(AESFSb[MBEDTLS_BYTE_0(*SK)]) ^ AES_RT1(AESFSb[MBEDTLS_BYTE_1(*SK)]) ^
                    AES_RT2(AESFSb[MBEDTLS_BYTE_2(*SK)]) ^ AES_RT3(AESFSb[MBEDTLS_BYTE_3(*SK)]);
        }
    }
