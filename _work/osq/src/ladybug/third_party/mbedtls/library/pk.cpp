...
int mbedtls_pk_sign_restartable(mbedtls_pk_context* ctx, mbedtls_md_type_t md_alg,
    ...
    if (rs_ctx != NULL && mbedtls_ecp_restart_is_enabled() && ctx->pk_info->sign_rs_func != NULL) {
        ...
        ret = ctx->pk_info->sign_rs_func(ctx->pk_ctx, md_alg, hash, hash_len, sig, sig_size,
            sig_len, f_rng, p_rng, rs_ctx->rs_ctx);
...
int mbedtls_pk_decrypt(mbedtls_pk_context* ctx, const unsigned char* input, size_t ilen,
    ...
    return (
        ctx->pk_info->decrypt_func(ctx->pk_ctx, input, ilen, output, olen, osize, f_rng, p_rng));
}
...
int mbedtls_pk_encrypt(mbedtls_pk_context* ctx, const unsigned char* input, size_t ilen,
    ...
    return (
        ctx->pk_info->encrypt_func(ctx->pk_ctx, input, ilen, output, olen, osize, f_rng, p_rng));
}
