...
static int pk_parse_key_sec1_der(mbedtls_ecp_keypair* eck, const unsigned char* key, size_t keylen,
    ...
    if (!pubkey_done &&
        (ret = mbedtls_ecp_mul(&eck->grp, &eck->Q, &eck->d, &eck->grp.G, f_rng, p_rng)) != 0) {
        mbedtls_ecp_keypair_free(eck);
        return (MBEDTLS_ERROR_ADD(MBEDTLS_ERR_PK_KEY_INVALID_FORMAT, ret));
    }
...
static int pk_parse_key_pkcs8_encrypted_der(mbedtls_pk_context* pk, unsigned char* key,
    ...
    if (mbedtls_oid_get_pkcs12_pbe_alg(&pbe_alg_oid, &md_alg, &cipher_alg) == 0) {
        if ((ret = mbedtls_pkcs12_pbe(&pbe_params, MBEDTLS_PKCS12_PBE_DECRYPT, cipher_alg, md_alg,
...
int mbedtls_pk_parse_key(mbedtls_pk_context* pk, const unsigned char* key, size_t keylen,
    ...
    if (key[keylen - 1] != '\0')
        ...
        ret = mbedtls_pem_read_buffer(&pem, "-----BEGIN RSA PRIVATE KEY-----",
            "-----END RSA PRIVATE KEY-----", key, pwd, pwdlen, &len);
    ...
    if (key[keylen - 1] != '\0')
        ...
        ret = mbedtls_pem_read_buffer(&pem, "-----BEGIN EC PRIVATE KEY-----",
            "-----END EC PRIVATE KEY-----", key, pwd, pwdlen, &len);
    if (ret == 0) {
        pk_info = mbedtls_pk_info_from_type(MBEDTLS_PK_ECKEY);

        if ((ret = mbedtls_pk_setup(pk, pk_info)) != 0 ||
            (ret = pk_parse_key_sec1_der(mbedtls_pk_ec(*pk), pem.buf, pem.buflen, f_rng, p_rng)) !=
                0) {
            mbedtls_pk_free(pk);
        }
...
int mbedtls_pk_parse_public_key(mbedtls_pk_context* ctx, const unsigned char* key, size_t keylen) {
    ...
    if (key[keylen - 1] != '\0')
        ...
        ret = mbedtls_pem_read_buffer(&pem, "-----BEGIN RSA PUBLIC KEY-----",
            "-----END RSA PUBLIC KEY-----", key, NULL, 0, &len);
