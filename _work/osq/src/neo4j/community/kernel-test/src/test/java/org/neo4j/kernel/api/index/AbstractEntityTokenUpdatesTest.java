...
abstract class AbstractEntityTokenUpdatesTest<
    ...
    void shouldNotGenerateUpdateForExistingToken(Entity entity) {
        T updates = forEntity(ENTITY_ID, false).withTokens(TOKEN_1_ID).build();
        assertThat(updates.tokenUpdateForIndexKey(entity.getTokenIndex())).isEmpty();
    }
    ...
    private enum Entity {
        NODE {
            ...
            IndexDescriptor getTokenIndex() {
                return IndexPrototype.forSchema(ANY_TOKEN_NODE_SCHEMA_DESCRIPTOR)
                        .withName("0")
                        .materialise(0);
            }
        },
        RELATIONSHIP {
            @Override
            IndexDescriptor getTokenIndex() {
                return IndexPrototype.forSchema(ANY_TOKEN_RELATIONSHIP_SCHEMA_DESCRIPTOR)
                        .withName("1")
                        .materialise(1);
            }
        };
