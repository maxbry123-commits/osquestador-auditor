...
public class HilbertSpaceFillingCurve3D extends SpaceFillingCurve {
    ...
    static class HilbertCurve3D extends CurveRule {
        ...
        private HilbertCurve3D rotateOneThirdDiagonalNeg(boolean direction) {
            ...
            for (int i = 0; i < length(); i++) {
                if (direction) {
                    newNpoints[i] =
                            BinaryCoordinateRotationUtils3D.xXOR(BinaryCoordinateRotationUtils3D.rotateNPointLeft(
                                    BinaryCoordinateRotationUtils3D.xXOR(npointValues[i])));
                } else {
                    newNpoints[i] =
                            BinaryCoordinateRotationUtils3D.xXOR(BinaryCoordinateRotationUtils3D.rotateNPointRight(
                                    BinaryCoordinateRotationUtils3D.xXOR(npointValues[i])));
                }
            }
            return new HilbertCurve3D(newNpoints);
    ...
    static class SubCurve3D {
        ...
        public boolean equals(Object obj) {
            ...
            return this.firstMove == other.firstMove
                    && this.secondMove == other.secondMove
                    && this.overallDirection == other.overallDirection;
        }
