 ...
 */
class Lucene10IndexSampler implements LuceneIndexSampler {
    private final Lucene10IndexSearcher indexSearcher;
    private final IndexSamplingConfig indexSamplingConfig;
    private final TaskCoordinator taskCoordinator;

    Lucene10IndexSampler(
            Lucene10IndexSearcher indexSearcher,
    ...
    private static Set<String> getFieldNamesToSample(LeafReaderContext readerContext) {
        ...
        reader.getFieldInfos().forEach(info -> {
            ...
            if (!LuceneDocumentsFactory.ENTITY_ID_KEY.equals(name)) {
                fieldNames.add(name);
            }
        });
        return fieldNames;
