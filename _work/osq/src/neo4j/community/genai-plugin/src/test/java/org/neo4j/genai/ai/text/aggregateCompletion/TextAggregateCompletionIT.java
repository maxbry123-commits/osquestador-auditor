...
class TextAggregateCompletionIT {
    ...
    void completionWithRequiredArgs() {
        for (final var conf : confRequired()) {
            assertNonBlankResult("""
                    WITH %s as conf
                    MATCH (u:UserReview)
                    WITH ai.text.aggregateCompletion(u.review, 'Hello, can you tell me the issues with my restaurant based on these reviews?', '%s', conf) as result
