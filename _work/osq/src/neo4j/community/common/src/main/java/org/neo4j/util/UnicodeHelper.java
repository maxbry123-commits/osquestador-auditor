...
public final class UnicodeHelper {
    ...
    // Implements part of Cypher 5 lexer (not sure why we do not use the real lexer here)
    private static class Cypher25 {
        static final CharRangeSet LETTER_CHARS = antlrRanges(
                // Copy from Cypher25Lexer.g4
        ...
        static final CharRangeSet LETTER_PART_CHARS = antlrRanges(
                // Copy from Cypher25Lexer.g4
        ...
        static boolean isLetter(int codepoint) {
            return LETTER_CHARS.contains(codepoint);
        }

        static boolean isLetterPart(int codepoint) {
            return LETTER_CHARS.contains(codepoint) || LETTER_PART_CHARS.contains(codepoint);
        }
    }

    // Implements part of Cypher 5 lexer (not sure why we do not use the real lexer here)
    private static class Cypher5 {
        static final char LETTER_CHAR_REMOVED_IN_25 = '\u2e2f';
        static final CharRangeSet LETTER_PART_REMOVED_IN_25 = new CharRangeSet(new char[] {
            0x0000, 0x0008,
            0x000e, 0x001b,
            0x0024, 0x0024, // $
            0x007f, 0x009f,
        ...
        static boolean isLetter(int codepoint) {
            return Cypher25.isLetter(codepoint) || codepoint == LETTER_CHAR_REMOVED_IN_25;
        }

        static boolean isLetterPart(int codepoint) {
            return Cypher25.isLetterPart(codepoint)
                    || LETTER_PART_REMOVED_IN_25.contains(codepoint)
                    || codepoint == LETTER_CHAR_REMOVED_IN_25;
        }
    }

    // Converts antlr character ranges to CharRangeSet
    // Higher planes are unsupported
    private static CharRangeSet antlrRanges(String antlr) {
        if (antlr.charAt(0) != '[' || antlr.charAt(antlr.length() - 1) != ']') {
