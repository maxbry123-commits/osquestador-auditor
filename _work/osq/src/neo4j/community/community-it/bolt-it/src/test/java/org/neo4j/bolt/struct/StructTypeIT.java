...
public class StructTypeIT {
    ...
    @ProtocolTest
    void shouldSendPoint2D(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testSendingOfBoltV2Value(wire, connection, pointValue(WGS_84, 39.111748, -76.775635));
    }

    @ProtocolTest
    void shouldReceivePoint2D(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testReceivingOfBoltV2Value(
                wire, connection, "RETURN point({x: 40.7624, y: 73.9738})", pointValue(CARTESIAN, 40.7624, 73.9738));
    }

    @ProtocolTest
    void shouldSendAndReceivePoint2D(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testSendingAndReceivingOfBoltV2Value(wire, connection, pointValue(WGS_84, 38.8719, 77.0563));
    }

    @ProtocolTest
    void shouldSendDuration(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testSendingOfBoltV2Value(wire, connection, duration(5, 3, 34, 0));
    }

    @ProtocolTest
    void shouldReceiveDuration(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testReceivingOfBoltV2Value(
                wire,
                connection,
    ...
    @ProtocolTest
    void shouldSendAndReceiveDuration(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testSendingAndReceivingOfBoltV2Value(wire, connection, duration(17, 9, 2, 1_000_000));
    }

    @ProtocolTest
    void shouldSendDate(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testSendingOfBoltV2Value(wire, connection, date(1991, 8, 24));
    }

    @ProtocolTest
    void shouldReceiveDate(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testReceivingOfBoltV2Value(wire, connection, "RETURN date('2015-02-18')", date(2015, 2, 18));
    }

    @ProtocolTest
    void shouldSendAndReceiveDate(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testSendingAndReceivingOfBoltV2Value(wire, connection, date(2005, 5, 22));
    }

    @ProtocolTest
    void shouldSendLocalTime(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testSendingOfBoltV2Value(wire, connection, localTime(2, 35, 10, 1));
    }

    @ProtocolTest
    void shouldReceiveLocalTime(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testReceivingOfBoltV2Value(wire, connection, "RETURN localtime('11:04:35')", localTime(11, 04, 35, 0));
    }

    @ProtocolTest
    void shouldSendAndReceiveLocalTime(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testSendingAndReceivingOfBoltV2Value(wire, connection, localTime(22, 10, 10, 99));
    }

    @ProtocolTest
    void shouldSendTime(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testSendingOfBoltV2Value(wire, connection, time(424242, ZoneOffset.of("+08:30")));
    }

    @ProtocolTest
    void shouldReceiveTime(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testReceivingOfBoltV2Value(
                wire, connection, "RETURN time('14:30+0100')", time(14, 30, 0, 0, ZoneOffset.ofHours(1)));
    }

    @ProtocolTest
    void shouldSendAndReceiveTime(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testSendingAndReceivingOfBoltV2Value(wire, connection, time(19, 22, 44, 100, ZoneOffset.ofHours(-5)));
    }

    @ProtocolTest
    void shouldSendLocalDateTime(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testSendingOfBoltV2Value(wire, connection, localDateTime(2002, 5, 22, 15, 15, 25, 0));
    }

    @ProtocolTest
    void shouldReceiveLocalDateTime(BoltWire wire, @Authenticated BoltTestConnection connection) throws Exception {
        testReceivingOfBoltV2Value(
                wire,
                connection,
    ...
    @ProtocolTest
    void shouldSendAndReceiveLocalDateTime(BoltWire wire, @Authenticated BoltTestConnection connection)
            throws Exception {
        testSendingAndReceivingOfBoltV2Value(wire, connection, localDateTime(1995, 12, 12, 10, 30, 0, 0));
    }

    @ProtocolTest
    void shouldSendDateTimeWithTimeZoneName(BoltWire wire, @Authenticated BoltTestConnection connection)
            throws Exception {
        testSendingOfBoltV2Value(wire, connection, datetime(1956, 9, 14, 11, 20, 25, 0, "Europe/Stockholm"));
    }

    @ProtocolTest
    void shouldReceiveDateTimeWithTimeZoneName(BoltWire wire, @Authenticated BoltTestConnection connection)
            throws Exception {
        testReceivingOfBoltV2Value(
                wire,
                connection,
    ...
    @ProtocolTest
    void shouldSendAndReceiveDateTimeWithTimeZoneName(BoltWire wire, @Authenticated BoltTestConnection connection)
            throws Exception {
        testSendingAndReceivingOfBoltV2Value(wire, connection, datetime(1984, 10, 11, 21, 30, 0, 0, "Europe/London"));
    }

    @ProtocolTest
    void shouldSendDateTimeWithTimeZoneOffset(BoltWire wire, @Authenticated BoltTestConnection connection)
            throws Exception {
        testSendingOfBoltV2Value(wire, connection, datetime(424242, 0, ZoneOffset.ofHoursMinutes(-7, -15)));
    }

    @ProtocolTest
    void shouldReceiveDateTimeWithTimeZoneOffset(BoltWire wire, @Authenticated BoltTestConnection connection)
            throws Exception {
        testReceivingOfBoltV2Value(
                wire,
                connection,
    ...
    @ProtocolTest
    void shouldSendAndReceiveDateTimeWithTimeZoneOffset(BoltWire wire, @Authenticated BoltTestConnection connection)
            throws Exception {
    ...
    @ProtocolTest
    public void shouldSendAndReceiveMapContainingStruct(BoltWire wire, @Authenticated BoltTestConnection connection)
            throws Exception {
    ...
    @ProtocolTest
    public void shouldSendAndReceiveListContainingStruct(BoltWire wire, @Authenticated BoltTestConnection connection)
            throws Exception {
