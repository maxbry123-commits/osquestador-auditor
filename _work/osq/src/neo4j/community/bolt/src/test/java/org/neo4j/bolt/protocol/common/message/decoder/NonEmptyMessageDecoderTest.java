...
public interface NonEmptyMessageDecoderTest<D extends MessageDecoder<?>> extends MessageDecoderTest<D> {
    ...
    default void shouldFailWithIllegalStructSizeWhenEmptyStructIsGiven() {
        ErrorGqlStatusObjectAssertions.assertThatThrownBy(() -> this.getDecoder()
                ...
                .hasMessage(ErrorUtil.useNewMessage(
                        ...
                        .whenLegacyFallbackTo("Illegal struct size: Expected struct to be "
                                + this.maximumNumberOfFields() + " fields but got 0"))
                ...
                .hasStatusDescription(String.format(
                        "error: data exception - invalid protocol type. Protocol type is invalid. Invalid number of struct components (received 0 but expected %s).",
                        this.maximumNumberOfFields()));
    }
}
