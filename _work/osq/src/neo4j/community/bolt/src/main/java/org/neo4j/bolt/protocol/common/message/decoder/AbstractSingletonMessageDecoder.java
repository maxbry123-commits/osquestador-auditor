...
public abstract class AbstractSingletonMessageDecoder<M extends RequestMessage> implements MessageDecoder<M> {
    ...
    @Override
    public M read(Connection connection, PackstreamBuf buffer, StructHeader header) throws PackstreamReaderException {
        PackstreamConditions.requireEmpty(header);

        return this.getMessageInstance();
    }
