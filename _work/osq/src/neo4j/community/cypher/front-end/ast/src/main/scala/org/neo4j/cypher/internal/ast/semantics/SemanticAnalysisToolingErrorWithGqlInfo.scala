...
object SemanticAnalysisToolingErrorWithGqlInfo {
  ...
  def invalidUseOfVariableLengthRelationshipError(
    expr: String,
    legacyMessage: String,
    position: InputPosition
  ): SemanticCheck =
    SemanticCheck.error(SemanticError.invalidUseOfVariableLengthRelationship(expr, legacyMessage, position))
}
