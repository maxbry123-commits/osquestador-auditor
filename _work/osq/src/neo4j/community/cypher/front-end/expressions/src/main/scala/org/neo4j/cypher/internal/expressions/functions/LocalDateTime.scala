...
/**
 * Note: This is a GQL alias to the `localdatetime` function.
 */
case object LocalDateTime extends Function {
  def name = "local_datetime"
  ...
  override val signatures: Vector[FunctionTypeSignature] = Vector(
    FunctionTypeSignature(
      ...
      outputType = CTLocalDateTime,
      description = "Creates a `LOCAL DATETIME` instant.",
      category = Category.TEMPORAL,
      overrideDefaultAsString = Some("local_datetime(input = DEFAULT_TEMPORAL_ARGUMENT :: ANY) :: LOCAL DATETIME"),
    ...
    FunctionTypeSignature(
      ...
      overrideDefaultAsString = Some(
        "local_datetime(input = DEFAULT_TEMPORAL_ARGUMENT :: ANY, pattern = DEFAULT_TEMPORAL_ARGUMENT :: STRING) :: LOCAL DATETIME"
      ),
      argumentDescriptions = Map(
        "input" ->
          "Either a string representation of a temporal value, a map containing the single key 'timezone', or a map containing temporal values ('year', 'month', 'day', 'hour', 'minute', 'second', 'millisecond', 'microsecond', 'nanosecond') as components.",
