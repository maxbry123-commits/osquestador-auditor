...
/**
 * Note: This is a GQL alias to the `time` function.
 */
case object ZonedTime extends Function {
  def name = "zoned_time"
  ...
  override val signatures: Vector[FunctionTypeSignature] = Vector(
    FunctionTypeSignature(
      ...
      outputType = CTZonedTime,
      description = "Creates a `ZONED TIME` instant.",
      category = Category.TEMPORAL,
      overrideDefaultAsString = Some("zoned_time(input = DEFAULT_TEMPORAL_ARGUMENT :: ANY) :: ZONED TIME"),
    ...
    FunctionTypeSignature(
      ...
      overrideDefaultAsString = Some(
        "zoned_time(input = DEFAULT_TEMPORAL_ARGUMENT :: ANY, pattern = DEFAULT_TEMPORAL_ARGUMENT :: STRING) :: ZONED TIME"
      ),
      argumentDescriptions = Map(
        "input" ->
          "Either a string representation of a temporal value, a map containing the single key 'timezone', or a map containing temporal values ('hour', 'minute', 'second', 'millisecond', 'microsecond', 'nanosecond', 'timezone') as components.",
        "pattern" ->
          "A pattern used to parse a zoned time value. If the pattern is not provided the value will be parsed according to default patterns."
      ),
      scopes = Set(CypherVersion.Cypher25)
    )
  )
