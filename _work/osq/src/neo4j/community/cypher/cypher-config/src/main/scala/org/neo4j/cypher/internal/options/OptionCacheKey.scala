...
object OptionCacheKey extends ProductDerivation[OptionCacheKey] {
   ...
   */
  override def join[T](caseClass: CaseClass[OptionCacheKey, T]): OptionCacheKey[T] =
    (value: T) =>
      caseClass.params
        .map(param => param.typeclass.cacheKey(param.deref(value)))
        .filterNot(_.isBlank)
        .mkString(" ")
}
