...
object asPrettyString {
  ...
  def apply(maybeVariable: Option[LogicalVariable]): PrettyString = maybeVariable.map(apply(_)).getOrElse(pretty"")

  /**
   * Remove autogenerated names/params and add backticks.
   */
  def apply(variableName: String): PrettyString = PrettyString(backtick(makePretty(variableName)))
