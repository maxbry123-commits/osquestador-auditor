...
def chunk_big(root):
    for p in list(root.rglob('*')):
        if not p.is_file() or p.stat().st_size<=CHUNK: continue
...
def safe_extract(z,dest):
    ...
    with zipfile.ZipFile(z) as zf:
        for info in zf.infolist():
            ...
            if target!=base and base not in target.parents: raise RuntimeError(f'FORENSIC FAIL: path traversal in {z}: {info.filename}')
        zf.extractall(dest)
def push(label):
    for attempt in range(1,8):
        try: run(['git','fetch','origin','main']); run(['git','rebase','origin/main']); run(['git','push','origin','HEAD:main']); print('PUSH PASS',label,attempt); return
...
if not LIST.exists(): raise SystemExit(f'FORENSIC FAIL: required queue missing: {LIST}')
REPOS=json.loads(LIST.read_text())
if len(REPOS)!=96 or [str(x['n']) for x in REPOS] != [f'{i:02d}' for i in range(1,97)]: raise SystemExit('FORENSIC FAIL: REPOS.json must contain contiguous 01..96 queue')
...
for item in REPOS:
    ...
    if full.stat().st_size<=SPLIT_TARGET:
        ...
        for i,p in enumerate(sorted(made,key=lambda p:(p.stat().st_mtime,p.name)),1): q=PACK/f'{slug}_{i:04d}.zip'; p.replace(q); parts.append((q,q.stat().st_size))
    for z,sz in parts:
    ...
    for z,sz in parts: shutil.copy2(z,dest/z.name); safe_extract(z,dest)
