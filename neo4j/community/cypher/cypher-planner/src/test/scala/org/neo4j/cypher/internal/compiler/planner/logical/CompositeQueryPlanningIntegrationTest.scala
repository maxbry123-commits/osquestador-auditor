...
class CompositeQueryPlanningIntegrationTest extends CypherPlannerTestSuite with LogicalPlanningIntegrationTestSupport {
  ...
  test("should plan a simple composite query with a MATCH clause") {
    ...
    val plan = planner.plan(CypherVersion.Legacy.legacyVersion(), query)
  ...
  test("should plan a composite query containing sub-queries") {
    ...
    planner.plan(CypherVersion.Legacy.legacyVersion(), query) shouldEqual planner
      .planBuilder()
  ...
  test("should plan a composite query containing union scoped sub-queries; CYPHER 5") {
    val query =
      """UNWIND [1,2,3] AS i
        |CALL {
        |  WITH i
    ...
    val plan = planner.plan(CypherVersion.Cypher5, query)
  ...
  test("should plan a composite query containing union scoped sub-queries; CYPHER 25") {
    val query =
      """UNWIND [1,2,3] AS i
        |CALL {
        |  WITH i
    ...
    plan shouldEqual planner
      .planBuilder(CypherVersion.Cypher25)
      .produceResults("product", "customer")
      .apply()
      .|.distinct("customer AS customer")
      .|.union()
