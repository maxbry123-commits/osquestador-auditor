...
case object ProjectNamedPaths extends Rewriter with StepSequencer.Step {
  ...
  case class Projectibles(
    ...
    def merge(that: Projectibles): Projectibles =
      copy(
        paths ++ that.paths,
        protectedVariables ++ that.protectedVariables,
        variableRewrites ++ that.variableRewrites,
        insertedWiths ++ that.insertedWiths,
        insertedImports ++ that.insertedImports
      )
    ...
    def withInsertedWith(query: SingleQuery, wizz: With): Projectibles = {
      if (insertedWiths.contains(query))
        this
      else
    ...
    def withInsertedImports(subquery: ScopeClauseSubqueryCall, imports: Seq[LogicalVariable]): Projectibles = {
      if (insertedImports.contains(subquery))
        this
      else
    ...
    def withVariableRewritesForExpression(expr: Expression): Projectibles =
      expr.folder.treeFold(self) {
        case subquery: FullSubqueryExpression =>
          acc => SkipChildren(collectProjectibles(subquery, acc))
        case ident: Variable =>
          acc =>
  ...
  private def collectProjectibles(input: AnyRef, initAcc: Projectibles = Projectibles.empty): Projectibles =
    input.folder.treeFold(initAcc) {
      ...
      case ProjectionClause(_, returnItems, _, _, _, _, _) =>
        acc =>
          ...
          TraverseChildrenNewAccForSiblings(
            ...
            _.withoutNamedPaths(namesReplaced)
          )
            ...
            case query: SingleQuery => innerAcc =>
                ...
                val newImportingWith: Option[With] = {
                  if (returnItemsWithVariablesFromPaths.isEmpty) {
                    ...
                    val returnItems: Seq[ReturnItem] =
                      (returnItemsWithVariablesFromPaths ++ nonPathReturnItems).distinct
                    Some(With(
                      distinct = false,
  ...
  private def collectRelationshipVarsInQpp(element: PatternElement): Seq[LogicalVariable] =
    element match {
      case rc @ RelationshipChain(_, RelationshipPattern(_, _, _, _, _, _), _) =>
        ...
        rc.allSingletonVariablesLeftToRight.init
