...
public enum GeometryType {
    ...
    GEOMETRY_POINT(1, "Point") {
        ...
        public ArrayValue decodeArray(GeometryHeader header, byte[] data) {
            ...
            if (dataValue instanceof FloatingPointArray numbers) {
                PointValue[] points = new PointValue[numbers.intSize() / header.dimension];
                ...
                throw new InvalidRecordException("Point array with unexpected type. Actual:"
                        + dataValue.getClass().getSimpleName() + ". Expected: FloatingPointArray.");
            }
        }
    };
    ...
    public static Value decode(long[] valueBlocks, int offset) {
        ...
        if (dimension > GeometryType.getMaxSupportedDimensions()) {
            throw new UnsupportedOperationException("Points with more than " + GeometryType.getMaxSupportedDimensions()
        ...
        for (int i = 1; i < points.length; i++) {
            if (dimension != points[i].coordinate().length) {
                throw new IllegalArgumentException(
                        "Attempting to store array of points with inconsistent dimension. Point " + i
                                + " has a different dimension.");
            }
            if (!crs.equals(points[i].getCoordinateReferenceSystem())) {
                throw new IllegalArgumentException("Attempting to store array of points with inconsistent CRS. Point "
                        + i + " has a different CRS.");
            }
        }
