...
func installWorkBuddy(env *setup.Environment) error {
	configDir := env.ConfigDir

	if !setupGlobal && !setupYes && setup.IsInteractive() {
		home := setup.HomeDir()
		localDir := ".workbuddy"
		globalDir := home + "/.workbuddy"
		idx := setup.SelectOne("Install scope",
			[]string{
				fmt.Sprintf("Local — this project only (%s/)", localDir),
				fmt.Sprintf("Global — all projects (%s/)", globalDir),
	...
	fmt.Println("\n[1/3] Skill")
	if path, err := setup.WorkBuddyWriteSkill(configDir); err != nil {
		setup.StatusError(0, 0, "Skill", err)
		return err
	} else {
		setup.StatusOK(0, 0, "Skill", path)
	...
	for _, hook := range []struct {
		...
		{"Hook: prime", "prime.sh", assets.WorkBuddyPrimeHook},
		{"Hook: remind", "user_prompt.sh", assets.WorkBuddyUserPromptHook},
		{"Hook: nudge", "stop.sh", assets.WorkBuddyStopHook},
	} {
		if path, err := setup.WorkBuddyWriteHook(configDir, hook.filename, hook.content); err != nil {
			setup.StatusError(0, 0, hook.label, err)
			return err
		} else {
			setup.StatusOK(0, 0, hook.label, path)
	...
	if path, err := setup.WorkBuddyRegisterHooks(configDir); err != nil {
		setup.StatusError(0, 0, "Settings", err)
		return err
	} else {
		setup.StatusUpdated(0, 0, "Settings", path)
	...
	fmt.Printf("  Prompts  %s/ (guide.md, skill.md)\n", promptPath)
	fmt.Println()
	fmt.Println("Restart WorkBuddy to activate the mnemon skill and hooks.")
	fmt.Println("Run 'mnemon setup --eject --target workbuddy' to remove.")
