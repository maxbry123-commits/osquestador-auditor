...
### Added
- **Grounding (anti-hallucination)**: `memory_ground` tool verifies claims have memory backing before an agent asserts them. Returns verified/ungrounded status with evidence.
- **Multi-context workspaces**: Load and query across multiple `.amem` files simultaneously.
  - `memory_workspace_create`: Create a workspace for cross-memory queries.
  - `memory_workspace_add`: Load an `.amem` file into a workspace with role (primary/secondary/reference/archive).
  - `memory_workspace_list`: List all contexts in a workspace.
  - `memory_workspace_query`: Search across all loaded memories.
  - `memory_workspace_compare`: Find where a topic exists/doesn't across contexts.
  - `memory_workspace_xref`: Cross-reference with coverage summary.
- 30 new V2 stress tests (grounding, workspace, integration).

### Changed
- SessionManager now includes WorkspaceManager for multi-context support.
...
### Fixed
- MCP shutdown lifecycle hardening: stdio transport now exits immediately after a successful `shutdown` request.
- Added protocol regression coverage to ensure shutdown acknowledgement is persisted and surfaced reliably.
